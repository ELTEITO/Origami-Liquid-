...
class DashboardController {
  ...
  renderStats(stats) {
    ...
    const filteredCards = statsCards.filter(c => !/pedido|ingreso|total pedido|valor promedio/i.test(c.title));

    statsGrid.innerHTML = filteredCards.map(stat => `
  ...
  renderRecentActivity(activities) {
    ...
    activities = Array.isArray(activities) ? activities.filter(a => a.entity !== 'orders') : activities;
        ...
        ${activities.map(activity => `
          <div class="activity-item">
            <div class="activity-icon">
              <i class="fa-solid ${activityIcons[activity.entity] || 'fa-info'}"></i>
            </div>
  ...
  renderTopCategories(categories) {
    ...
    if (!categories || categories.length === 0) {
      container.innerHTML = '<p class="empty-state">No hay datos de categorías</p>';
      return;
    }
          ...
          return `
            <div class="category-item">
              <div class="category-info">
                ...
                <span class="category-count">${category.count} producto${category.count !== 1 ? 's' : ''}</span>
              </div>
  ...
  exportAllData() {
    try {
      ...
      a.href = url;
      a.download = `origami-backup-${new Date().toISOString().split('T')[0]}.json`;
