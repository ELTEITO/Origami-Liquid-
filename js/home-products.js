...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (async function loadHomeProducts() {
    try {
      ...
      container.innerHTML = '<h2 style="flex-basis:100%; margin:0 0 1rem 0;">Nuestros Equipos</h2>';
      ...
      const cards = products.slice(0, 3).map((p) => {
        ...
        h3.textContent =
          `${p.Marca || p.marca || ""} ${p.Modelo || p.modelo || ""}`.trim() ||
          p.Modelo ||
          p.modelo ||
          "Producto";
      ...
      if (cards.length === 0) {
        ...
        verMasBtn.style.cssText = "display:inline-block; padding:0.75rem 1.5rem; color:var(--text-color); text-decoration:none; font-weight:500; border-radius:8px; transition:background 0.3s ease;";
